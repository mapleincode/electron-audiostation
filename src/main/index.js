    ...
    try {
      (async () => {
        ...
        const settingsWindow = BrowserWindow.getAllWindows().find(
          (win) => win.getTitle() === (isCurrentZhCN ? "设置" : "Settings")
        );
...
function createWindow() {
  ...
  mainWindow.on("close", (event) => {
    ...
    if (closeAction === "minimize") {
      ...
      const options = {
        type: "question",
        buttons: [dialogTexts.minimize, dialogTexts.exit, dialogTexts.cancel],
        defaultId: 0,
...
function createTray() {
  ...
  try {
    // 1. 创建固定宽度的图标（22x22 推荐）
    let icon = nativeImage.createFromPath(
      path.join(
        __dirname,
        isMac ? "../../assets/icon.mac.png" : "../../assets/icon.png"
      )
    );
      ...
      { type: "separator" },
      {
        ...
        click: () => {
          mainWindow.webContents.executeJavaScript(
            SynologyAudioStationControlScript.play
          );
        },
      },
      {
        ...
        click: () => {
          mainWindow.webContents.executeJavaScript(
            SynologyAudioStationControlScript.stop
          );
        },
      },
      {
        ...
        click: () => {
          mainWindow.webContents.executeJavaScript(
            SynologyAudioStationControlScript.previous
          );
        },
      },
      {
        ...
        click: () => {
          mainWindow.webContents.executeJavaScript(
            SynologyAudioStationControlScript.next
          );
        },
      },
      {
        ...
        click: () => {
          mainWindow.webContents.executeJavaScript(
            SynologyAudioStationControlScript.volumeUp
          );
        },
      },
      {
        ...
        click: () => {
          mainWindow.webContents.executeJavaScript(
            SynologyAudioStationControlScript.volumeDown
          );
        },
      },
      { type: "separator" },
        ...
        submenu: [
          {
            ...
            click: async () => {
              await shell.openExternal(
                "https://github.com/sdjnmxd/electron-audiostation"
              );
            },
          },
          {
            ...
            click: async () => {
              await shell.openExternal(
                "https://github.com/sdjnmxd/electron-audiostation/issues"
              );
            },
          },
          { type: "separator" },
...
function openLogViewer() {
  try {
    ...
    // 检查是否已经存在日志查看器窗口
    const existingLogViewer = BrowserWindow.getAllWindows().find(
      (win) => win.getTitle() === logViewerTitle
    );
...
function showSettings() {
  try {
    ...
    // 检查是否已经存在设置窗口
    const existingSettingsWindow = BrowserWindow.getAllWindows().find(
      (win) => win.getTitle() === settingsTitle
    );
